...
const AdviserAuditTrail: React.FC = () => {
  ...
  // Auth & Assigned Section Fetching
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          setUserName(`${data.firstName || ""} ${data.lastName || ""}`.trim() || "Adviser");

          // Retrieve Adviser's Assigned Sections dynamically from existing user database record
          ...
          onSnapshot(notifQ, (snap) => setUnreadNotificationCount(snap.size));
        }
      } catch (error) {
        console.error("Adviser audit trail auth error:", error);
      }
    });
    return () => unsub();
  }, [navigate]);

  // Firestore Real-Time Query strictly scoped to Adviser's Assigned Sections
  useEffect(() => {
    ...
    // Determine target sections: query only allowed assigned sections
    const targetSections = selectedSectionFilter === "ALL"
      ? adviserSections
    ...
    // Backend/Database query RESTRICTED to assigned sections
  ...
  // Filtered Logs for client-side search, action, and date filtering
    ...
    <div className="flex min-h-screen bg-gray-50/30 overflow-hidden font-sans">
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-6 overflow-y-auto">
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 px-2">Main Menu</p>
            <div className="space-y-1">
              ...
              <button
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold bg-[#c9a654] text-white shadow-md transition-all"
              >
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 px-2">Account</p>
            <div className="space-y-1">
              <button
      ...
      {/* MAIN CONTENT AREA */}
      <main
        className={`flex-1 transition-all duration-300 ease-in-out min-h-screen overflow-y-auto ${
      ...
      >
        ...
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
          {/* HEADER & ASSIGNED SECTIONS BANNER */}
          <div>
            <h1 className="text-3xl font-bold text-[#122244]">Audit Trail</h1>
            <p className="text-sm text-gray-500 mt-1">
              View system activity and track modifications made across your assigned academic sections.
            </p>
          </div>

          {/* DYNAMIC ASSIGNED SECTIONS CARD */}
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-xs">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4 text-[#c9a654]" /> Your Assigned Sections
                </p>
                <div className="flex flex-wrap items-center gap-2">
                  {adviserSections.length === 0 ? (
                    <span className="text-xs text-gray-400 font-medium italic">No sections currently assigned</span>
                  ) : (
                    adviserSections.map((sec) => (
                      <span
                        key={sec}
                        className="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-50 text-[#122244] border border-amber-200/60 shadow-2xs"
                      >
                        <Tag className="w-3 h-3 text-[#c9a654] mr-1.5" />
                        {sec}
                      </span>
                    ))
                  )}
              ...
              <div className="text-xs text-gray-400 font-medium bg-gray-50 px-3 py-2 rounded-xl border border-gray-100 flex items-center gap-2 self-start md:self-auto">
                <Clock className="w-3.5 h-3.5 text-gray-500" /> Real-time activity logs
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-xs space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {/* SECTION FILTER - STRICTLY RESTRICTED TO ASSIGNED SECTIONS */}
              <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search description, user name, or section..."
                value={searchTerm}
                  ...
                  {isLoading ? (
                    Array.from({ length: 6 }).map((_, i) => (
                      <tr key={i}>
                        ...
                        <td className="px-5 py-4"><Skeleton width={220} /></td>
                        <td className="px-5 py-4"><Skeleton width={110} /></td>
                    ...
                    <tr>
                      <td colSpan={6} className="px-5 py-12 text-center text-gray-400">
                        ...
                        <p className="font-semibold text-gray-600">No audit records found</p>
                        <p className="text-[11px] text-gray-400 mt-1">
                          No activity records exist matching your active section filters.
                        </p>
                      </td>
                    </tr>
                  ) : (
...
export default AdviserAuditTrail;
