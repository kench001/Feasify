...
const AdviserNotifications: React.FC = () => {
  ...
  const setupNotificationsListener = (uid: string): (() => void) => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      console.error("Adviser notifications listener error:", error);
      setNotifications([]);
    });
    return unsub;
  };
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'message': return <div className="p-2 bg-purple-50 rounded-full"><Users className="w-5 h-5 text-purple-500" /></div>;
      case 'group': return <div className="p-2 bg-green-50 rounded-full"><Folder className="w-5 h-5 text-green-500" /></div>;
    ...
    <div className="flex min-h-screen bg-gray-50/50">
              ...
              <div className="pl-4 pr-2 py-2 space-y-1.5">
                {adviserSections.map((sectionName) => (
                  <button key={sectionName} onClick={() => { navigate(`/adviser/dashboard?section=${encodeURIComponent(sectionName)}`); }}
                    className={`w-full text-left text-sm px-3 py-2 rounded-lg transition-all ${activeSection === sectionName ? 'bg-[#c9a654] text-white font-bold shadow-sm' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                    {sectionName}
                  </button>
                ))}
              </div>
      ...
      <main className={`flex-1 transition-all duration-300 ease-in-out min-h-screen ${isSidebarOpen ? 'lg:ml-64' : 'ml-0'}`}>
        <div className="bg-white border-b border-gray-100 p-4 flex items-center gap-2 text-sm text-gray-500 sticky top-0 z-10">
          ...
          <span className="mx-2">|</span><span className="cursor-pointer hover:text-[#c9a654] font-semibold text-gray-900" onClick={()=>navigate('/adviser/dashboard')}>FeasiFy</span><span>›</span><span className="font-semibold text-gray-900">Notifications</span>
        </div>

        <div className="p-6 md:p-8 max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-extrabold text-[#122244]">Notification</h1>
