...
const AdviserNotifications: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'group': return <div className="p-2 bg-green-50 dark:bg-green-900/30 rounded-full"><Folder className="w-5 h-5 text-green-500 dark:text-green-400" /></div>;
      default: return <div className="p-2 bg-gray-50 dark:bg-gray-800 rounded-full"><Bell className="w-5 h-5 text-gray-500 dark:text-gray-400" /></div>;
    }
  };

  const getInitials = (name: string) => name ? name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2) : "A";

  const filteredNotifications = notifications.filter(n => activeTab === 'unread' ? !n.isRead : n.isRead);

  return (
    <div className="flex min-h-screen bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-200">
      {/* SIDEBAR */}
      <aside className={`hidden lg:flex w-64 bg-[#122244] dark:bg-[#0f172a] border-r border-transparent dark:border-gray-800 text-white flex-col fixed inset-y-0 shadow-xl z-20 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 p-4 space-y-8 mt-4">
          ...
          <div>
            ...
            <div className="space-y-1">
              <button onClick={() => navigate("/adviser/profile")} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-white/10 dark:hover:bg-gray-800 transition-all"><User className="w-4 h-4" /> Profile</button>
              ...
              <button onClick={() => navigate("/")} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-white/10 dark:hover:bg-gray-800 transition-all"><ShieldAlert className="w-4 h-4" /> Logout</button>
            </div>
          </div>
        </nav>
        <div className="p-4 border-t border-white/10 dark:border-gray-800 bg-black/20 dark:bg-gray-900/50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate text-white">{userName}</p><p className="text-[10px] text-gray-400 truncate">Feasibility Adviser</p></div>
            <button
              onClick={() => navigate('/adviser/notifications')}
              className="p-2 text-gray-300 hover:text-white hover:bg-white/10 dark:hover:bg-gray-800 rounded-lg transition-all relative shrink-0"
              title="Notifications"
            >
      ...
      {/* MAIN CONTENT */}
      <main className={`flex-1 transition-all duration-300 ease-in-out min-h-screen bg-gray-50 dark:bg-gray-900 ${isSidebarOpen ? 'lg:ml-64' : 'ml-0'}`}>
        <div className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 p-4 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 sticky top-0 z-10 transition-colors">
          <SidebarIcon className="w-4 h-4 cursor-pointer hover:text-gray-800 dark:hover:text-gray-200 transition-colors" onClick={() => setIsSidebarOpen(!isSidebarOpen)} />
          <span className="mx-2 text-gray-300 dark:text-gray-600">|</span>
          <span className="cursor-pointer hover:text-[#c9a654] dark:hover:text-[#c9a654] font-semibold text-gray-900 dark:text-gray-100 transition-colors" onClick={()=>navigate('/adviser/dashboard')}>FeasiFy</span>
        ...
        <div className="p-6 md:p-8 max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-extrabold text-[#122244] dark:text-white">Notification</h1>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden transition-colors">
            {/* Toolbar */}
            <div className="p-4 border-b border-gray-50 dark:border-gray-700 flex items-center justify-between bg-gray-50/30 dark:bg-gray-900/50">
              <div className="flex items-center gap-3 px-2">
                ...
                />
                <span className="text-sm font-bold text-gray-700 dark:text-gray-300">Select All</span>
              </div>
            ...
            <div className="divide-y divide-gray-50 dark:divide-gray-700">
              {filteredNotifications.length === 0 ? (
                ...
                <>
                  <div className="px-6 py-3 bg-gray-50/50 dark:bg-gray-900/50 text-[10px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-[0.2em]">Today</div>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start">
                          <h3 className="font-bold text-[#122244] dark:text-gray-100 text-sm">{notif.title}</h3>
                          <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-tighter">{notif.timestamp}</span>
                        </div>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 leading-relaxed">{notif.message}</p>
                      </div>
                      <button className="p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors self-center">
