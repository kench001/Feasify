...
const ChairpersonNotifications: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'group': return <div className="p-2 bg-green-50 rounded-full"><Archive className="w-5 h-5 text-green-500" /></div>;
    ...
    <div className="flex min-h-screen bg-gray-50/50">
        ...
        <nav className="flex-1 p-4 space-y-8 mt-4">
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              <button onClick={() => navigate("/")} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-white/10 transition-all">
      ...
      <main className={`flex-1 transition-all duration-300 ease-in-out min-h-screen ${isSidebarOpen ? 'lg:ml-72' : 'ml-0'}`}>
        <div className="bg-white border-b border-gray-100 p-4 flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="mx-2">|</span><span className="cursor-pointer hover:text-[#c9a654]" onClick={()=>navigate('/admin/users')}>FeasiFy</span><span>›</span><span className="font-semibold text-gray-900">Notifications</span>
        </div>
